...
if [ -n "$FUSER_URING_FLAGS" ]; then
  ...
  PROBE_PID=$!
  # Requests block until the queues are registered, so the line is there once mountpoint's
  # stat was answered; a mount that never answers fails the run like a missing line does
  DEADLINE=$((SECONDS + 30))
