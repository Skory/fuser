...
impl ReplySender {
    ...
    /// Sends a data reply whose payload `f` writes into a zeroed buffer of `max_len` bytes,
    /// returning how many it filled. The ring writes into the entry's own buffer when it can;
    /// otherwise a heap buffer is filled and sent. Exactly one reply is sent whatever `f`
    /// does: a count beyond the buffer is answered with `EINVAL`, a panic with `EIO`.
        ...
        F: FnOnce(&mut [u8]) -> Result<usize, Errno>,
    {
        ...
        // The guard stays armed until the reply is out, so a panic anywhere before that
        // (including framing a count `fuse_out_header.len` cannot hold) still answers EIO
...
impl ReplyRaw {
    ...
    /// Reply with data `f` writes into a buffer of `max_len` bytes. The sender answers a panic
    /// in `f` itself, so it is taken out first and `Drop` has nothing left to do.
...
mod test {
    ...
    /// A count short of the buffer sends that many bytes, and bytes the closure reported but
    /// did not write go out as zeros
    #[test]
    ...
    /// A count beyond the buffer is a caller bug answered with EINVAL, not sent
    #[test]
    ...
    /// A panic in the closure still gets the request exactly one reply, EIO, and no other
    #[test]
