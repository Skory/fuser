...
impl<'a> RequestWithSender<'a> {
    ...
    /// Wraps an already parsed request.
    ...
    /// The sender for a reply to this request. Every reply object, including the directory
    /// replies built outside `reply()`, obtains its sender here, so this is where the
    /// transport learns that a reply is coming.
