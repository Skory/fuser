...
impl RingEntry {
    ...
    unsafe fn write_reply(&self, commit_id: u64, iov: &[IoSlice<'_>]) {
        ...
        // `payload_cap` fits a u32 (`RingMemory::new`), so a fitting payload converts
...
impl Ring {
    ...
    /// Protocol step 4: the buffers are written, queue the entry for the ring thread.
    ...
    fn serve(self: &Arc<Self>, io: &mut RingIo, handler: &mut dyn FetchHandler) -> io::Result<()> {
        ...
        loop {
            ...
            for (ud, res, flags) in cqes {
                ...
                // Only a counted command may move the counts; anything else is logged and dropped
...
pub(crate) mod test {
    ...
    /// For tests that check the mapping is gone after drop, which hold `UNMAP_CHECK`: the
    /// kernel fills holes top-down, so a 1 GiB (virtual) stride keeps the small mappings of
    /// the other tests away from `base`. `None` when the host refuses the reservation
    ...
    /// Marks entry `idx` as fetched and in the kernel's hands no more, as `handle_fetch` does
    ...
    /// Marks entry `idx` as fetched and dispatched, with a handle for it
    ...
    fn register_and_commit_sqe_encoding() {
        ...
        // The iovecs describe the header and the payload of this entry's stride
    ...
    /// The ring tests skip (with a captured `eprintln!`) when io_uring is unavailable; this is
    /// the one named failure that says so, e.g. under `kernel.io_uring_disabled=2` or a seccomp
    ...
    /// A `Session::from_fd` session dropped before `run`: the thread returns at once with the
    /// commands still counted, and the mapping is leaked
    #[test]
    ...
    fn push_or_submit_batches_when_the_queue_is_full() {
        ...
        // The fatal path is not exercised here; the mapping is leaked with in_kernel != 0
    }
    ...
    impl Served {
        ...
        /// Commits from a foreign thread and waits for the ring thread to flush it. The 2 s
        /// `recv_timeout` is the correctness bound: without a wake nothing would ever flush
        ...
        /// Waits (up to 2 s) until the pushed commit was rejected by the device and the entry
        /// was retired
    ...
    fn eventfd_wakes_the_ring_thread() {
        ...
        // The written buffers carry the out header and an empty payload
    ...
    fn cancelled_wake_poll_degrades_to_timed_waits() {
        ...
        // Bounded by 2 s; the count is what proves the poll failure was seen
        ...
        // First cancellation: the poll is re-armed once and a foreign commit still wakes
        ...
        // Second cancellation: the poll is dead, the wait is timed, commits still flush
        served.ring.hooks.inject.lock().push(cancel());
        served.wake();
        poll_failures(2);
    ...
    /// With a fatal error recorded but a request still held by the filesystem, the ring keeps
    /// serving until that reply is handed back; leaving earlier would drop it
    #[test]
    fn fatal_does_not_exit_while_a_request_is_outstanding() {
        ...
        *ring.hooks.flushed.lock() = Some(tx);
        ...
        // Wait until the loop has evaluated the exit test and decided to stay
        let deadline = Instant::now() + Duration::from_secs(2);
        ...
        // The commit was rejected by the device, the entry retired, nothing is held: exit
    ...
    fn cqe_classification() {
        ...
        ring.ring_thread.set(thread::current().id()).ok();
        // -EAGAIN/-EINTR resubmit the last command, which the device rejects; -ECONNABORTED
        // ends the connection; the ring drains and leaves
    ...
    /// A CQE for an entry that is not `InKernel` names no counted command and must not move
    /// the counts or reach the handler
    #[test]
    ...
    /// `-ENOTCONN` alone is a normal end: no fatal error, `thread_main` returns `Ok`
    #[test]
    ...
    fn fail_entry_keeps_the_first_error() {
        ...
        // A positive result on an entry is fatal with a descriptive error
        let ring = fake_ring(1, true);
        set_in_kernel(&ring, 0, 0);
    ...
    /// A commit from inside the handler for a request with a payload defers, and step 4
    /// finishes it after dispatch; the whole thing must not self-deadlock
    #[test]
    ...
    /// A request without payload is replied directly from inside the handler, and a handler
    /// that takes a reply object but does not reply leaves the entry `Dispatched`
    #[test]
    ...
    /// A foreign thread between commit steps 1 and 4 when dispatch returns leaves the entry
    /// `Committing`; the ring thread must leave it alone
    #[test]
    ...
    /// A commit racing the exit decision (connection gone, nothing in the kernel) either
    /// lands before it, in which case the ring thread waits for its CQE, or after it, in
    /// which case it is dropped; never in between. `live` is held rather than `state` because
    /// `live` is the lock the exit decision takes
    #[test]
    ...
    fn write_errno_and_hand_off_then_duplicate_commit_errno() {
        ...
        *e.state.lock() = EntryState::Committing;
        ...
        // commit_errno from Dispatched goes through step 1 and writes the header
