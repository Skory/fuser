...
pub(crate) mod test {
    ...
    /// which is what lets Miri check the aliasing discipline rather than the fixture
    ...
    impl FakeEntry {
        ...
        /// Writes `bytes` at `offset` through the raw pointer
        ...
        /// A copy of the stride bytes at `range`
    ...
    /// Stage, reply directly into the header and payload, then let the "kernel" refill the
    /// entry: with only raw pointers over the stride there is no long-lived reference for
    /// Miri to flag (`cargo +nightly miri test --features io-uring staging::`)
    #[test]
