...
impl<FS: Filesystem> Session<FS> {
    ...
    fn handshake(&mut self) -> io::Result<()> {
        ...
        loop {
            ...
            // The flag is requested only once the rings it commits the kernel to exist
            #[cfg(all(feature = "io-uring", target_os = "linux"))]
...
enum Event {
    ...
    /// during the unwind, and the thread keeps its queues served with EIO until the
    /// connection ends.
    Panicked,
}
...
mod test {
    ...
    /// `n_threads: Some(0)` is refused up front, whichever transport would serve the session,
    /// rather than by `run`
    #[test]
    ...
    /// Asking for the ring in a build that cannot serve one is an error, not a silent
    /// fallback, from both constructors and before anything is mounted
    #[test]
    ...
    /// A root with one 5000-byte file whose `read` answers through `ReplyData::fill`, on the
    /// calling thread or from a spawned one, or panics inside the closure
    struct FillFs {
        foreign: bool,
        panic: bool,
    ...
    /// `read` answered through `ReplyData::fill` over `/dev/fuse`, where the buffer is on the
    /// heap: the bytes come back intact, whole, in part and at the end of the file, whether
    /// the reply is made on the session thread or from a spawned one
    #[test]
    ...
    /// A panic inside a `fill` closure over `/dev/fuse`: the request gets EIO, and the session
    /// ends as after any other callback panic with the filesystem destroyed once and unmounted
    #[test]
    ...
    /// fusectl, mounted when root finds it missing and unmounted again when the last holder in
    /// this process drops, only if a holder mounted it; the tests that abort a connection run
    /// concurrently on a host that does not mount it
    ...
    impl Fusectl {
        /// `None` when fusectl is neither mounted nor mountable
...
mod uring_test {
    ...
    /// Records every log line of the process with the name of the thread that wrote it; the
    /// ring tests hold `SERIAL` while they read it, so the ring lines they see are their own.
    /// Lines are echoed when `RUST_LOG` is set
    ...
    /// Waits up to 5 s for `cond`, which observes work of another thread
    ...
    /// The captured lines of the session's threads, for a failure message
    ...
    /// `None` with the reason when the kernel or this environment cannot serve a ring
    ...
    /// Waits up to 5 s for `n` threads named `prefix*`; `run` spawns them from `fuser-bg`
    ...
    /// Waits up to `timeout` for the ring threads of this process to be gone
    ...
    /// The test filesystem: a root with `hello.txt`, `other.txt` and a 4 MiB `big.bin`, every
    /// attribute uncached so that each `stat` reaches it. The switches below make it reply
    ...
    /// Replies from a spawned thread that exits right after
    ...
    impl Filesystem for RingFs {
        ...
        /// The test answers once the ring thread has served another request, so the reply
        /// is a direct write into a `Dispatched` entry whose request carried a name payload
    ...
    /// A mounted ring session and the directory it lives in
    struct Mounted {
    ...
    impl Mounted {
        ...
        /// Checks nothing was left behind and releases the directory
        fn finish(mut self) {
            assert!(
    ...
    /// stat, readdir, read, write, getxattr (a name payload answered with data, the deferred
    /// path), unlink answered from another thread, and a dropped reply all go through the ring
    #[test]
    ...
    /// The reads every `fill` mount test makes: a small file, a 4 MiB file of many requests
    /// and a short read at the end of the small one
    ...
    /// `read` answered through `ReplyData::fill`, from the ring thread and from a spawned
    /// thread: the bytes come back intact, and every one of the closures wrote straight into
    /// its ring entry
    #[test]
    fn fill_serves_reads() {
        let _serial = serial();
    ...
    /// A panic inside a `fill` closure on the ring thread: the read fails with EIO instead of
    /// hanging on an entry the kernel never gets back, the session ends as after any other
    /// callback panic, and the guard's reply is the only one, so nothing is logged as unsent
    /// or duplicate
    #[test]
    ...
    /// A panic inside a `fill` closure on a foreign thread: only there does the guard's
    /// re-arming matter, because the session survives. The read gets EIO, the entry goes back
    /// to the kernel and the mount keeps serving
    #[test]
    ...
    /// Two rings: the queues are split between them and both serve
    #[test]
    ...
    /// A reply from a thread that exits at once must not hang the next request from the same
    /// CPU: the kernel delivers into an entry as task work of the thread that submitted its
    /// command, so every command must come from the ring thread, never from the replier
    #[test]
    ...
    /// The thread that constructs the session may be gone by the time it runs: the queues
    /// were registered by the ring threads, not by it
    #[test]
    ...
    /// A foreign reply on an idle ring is delivered through the eventfd at once; one made while
    /// the ring thread is inside a filesystem method goes out when that method returns, since
    /// only the ring thread submits
    #[test]
    ...
    /// From the INIT reply on, every request waits for the queues; `Session::new` returns with
    /// them registered, so a request made before `run` waits for `run`, not for fuser
    #[test]
    ...
    /// A mounted session dropped before `run` unmounts, and its ring threads drain to the end
    /// of the connection on their own: a request fetched before the drop is answered EIO
    #[test]
    ...
    /// A `from_fd` session dropped before `run` has no unmount to end its connection; the ring
    /// thread abandons its commands so the last `/dev/fuse` reference goes and the kernel
    /// aborts the connection, as it does without a ring
    #[test]
    ...
    /// The kernel posts no CQE at unmount for an entry held by userspace. With a depth-1 ring
    /// and a request held on every CPU's queue nothing of the ring's is in the kernel when the
    /// connection is aborted, so only the session's shutdown signal can end the ring thread;
    /// `umount_and_join` must still return within its window
    #[test]
    ...
    /// The ring twin of `test::panic_in_callback_ends_the_session`: a panic in a callback on
    /// a ring thread ends the session with the same error, the request gets EIO, and the
    /// filesystem is unmounted and destroyed exactly once
    #[test]
    ...
    /// Replies the ring thread dropped while a callback unwound, once at least one was
    /// logged; the `/dev/fuse` panic test logs the same line from its own reader thread
    ...
    /// `stat` from another thread, so a mount nobody answers fails the test instead of
    /// wedging it; the connection is then aborted by `Mounted::drop`
    ...
    /// Under `Session::spawn` the mount outlives `run`: after a callback panic the ring
    /// answers EIO without entering the filesystem again, until the `BackgroundSession` goes
    #[test]
    ...
    /// A depth the largest ring cannot hold is not a constructor error but a fallback, decided
    /// before the INIT reply and before anything was reserved
    #[test]
    ...
    /// A reply refused because the connection ended is expected during teardown and is not
    /// reported as an error; any other refusal still is
    #[test]
    ...
    /// The ring twin of `test::session_ends_cleanly_after_abort`: with `FUSE_ABORT_ERROR`
    /// negotiated the kernel completes the ring's idle commands with `-ECONNABORTED`, which is
    /// the end of the connection and not an error
    #[test]
    ...
    /// The runtime tests above skip quietly where the kernel cannot serve a ring. Where it
    /// advertises the transport they must have run, or a sandbox without io_uring would pass
    /// this module with no kernel coverage at all
    #[test]
    ...
    /// `Session::from_fd` over a socketpair standing in for the kernel: the test writes an
    /// INIT request offering `flags`, optionally closes its end first so the INIT reply cannot
    /// be written, and returns the session result and its end of the pair
    ...
    /// A kernel that does not offer `FUSE_OVER_IO_URING` gets a `/dev/fuse` session with a
    /// warning, and the INIT reply does not echo the flag
    #[test]
    ...
    /// Decision 24: an INIT reply that could not be written fails `Session::from_fd` when it
    /// echoed the flag, since nothing can be registered against it; without a ring the
    /// failure is logged and left to the event loop
    #[test]
