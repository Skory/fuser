//! Transport benchmark: the `bench_fs` example over `/dev/fuse` and over io_uring
//!
//! Each transport gets its own mount of `bench_fs`, built in release with the `io-uring`
//! feature, and runs the same workloads: `dd` streams at several block sizes over 1 GiB in
//! both directions, then `stat` and `pread` loops from one and from eight client threads.
//! Every workload is repeated; the table shows the median and the min-max spread per
//! transport. Runs on the host as root; the numbers depend on the machine, so compare
//! transports within one run only.
...
/// Between columns, so cells that fill their width still do not touch.
const GUTTER: &str = "  ";
...
pub(crate) async fn run_transport_bench(args: TransportBenchArgs) -> anyhow::Result<()> {
    ...
    match &client_cpus {
        Some(cpus) => {
            // dd and the small-op threads inherit the affinity of this process
...
/// Mounts `bench_fs` over `transport`, runs the workloads and unmounts. Fails with the mount
/// log attached if the session fell back to `/dev/fuse` although the ring was requested, so
/// a column labelled io_uring never shows `/dev/fuse` numbers.
