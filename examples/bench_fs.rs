...
//! reads and writes do no work beyond touching the payload, so the request round trip
//! dominates. `fuser-tests transport-bench` mounts it over `/dev/fuse` and over io_uring and
//! runs the same load against each.
//!
//! Attributes have a zero TTL, so every `stat` reaches the filesystem as a lookup and a
//! getattr, and the file is opened with `FOPEN_DIRECT_IO`, so every `read` does too.
